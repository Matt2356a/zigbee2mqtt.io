...
There is a serious issue when pairing the Osram Lightify Flex that leads to "unknow device" or "device not supported".
Just start pairing and when zigbee2mqtt started "interviewing the device" and then hangs for >10sek, just stay in the pairing process and unplug the device to power it off and plug in again. The interview process then finish correctly and the device will be recognized correctly.
